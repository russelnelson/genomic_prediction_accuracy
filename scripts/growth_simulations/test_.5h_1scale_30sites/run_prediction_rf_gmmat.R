...
# prediction
pr_gp <- pred(genotypes, meta, phenotypes = phenotypes, prediction.program = "BGLR", prediction.model = "BayesB", chain_length = 10000, burnin = 1000,  thin = 100, maf.filt = F, par = 24)
saveRDS(pr_gp, "../../../results/test_data/sites_30_scale_1_h_.5/hsd.05/trial_1/pr_gp.RDS")
rm(pr_gp)

pr_rf <- pred(genotypes, meta, phenotypes = phenotypes, prediction.program = "ranger", ntree = 500, prediction.model = "RJ", maf.filt = F, par = 24)
...
pr_gmmat <- pred(genotypes, phenotypes = phenotypes,
                 ...
                 maf.filt = F, runID = "gmmat_real", par = 24)
